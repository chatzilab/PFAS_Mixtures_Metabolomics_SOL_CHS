...
saveRDS(c18_mwas_results, file = here::here('Temporary results','1_1_chs_c18_mwas.rds'))
...
saveRDS(hilic_mwas_results, 
        file = here::here('Temporary results','1_1_chs_hilic_mwas.rds'))
...
exposure_merge_n_save = function(exposures){
  c18_data = 
    ...
    ungroup() %>%
    dplyr::select(m.z,r.t ,beta, t.score, p.value,
                  beta, conf.low, conf.high, mode)
