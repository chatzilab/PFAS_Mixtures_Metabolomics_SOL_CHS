...
mwas_results_c18 <- read_rds(fs::path(dir_temp, "1_1_SOLAR_c18_mwas.rds")) #%>% 
  # left_join(solar_c18_rsd)


mwas_results_hilic <- read_rds(fs::path(dir_temp, "1_1_SOLAR_hilic_mwas.rds")) #%>% 
  # left_join(solar_hilic_rsd)
...
solar_mzkey_w_superpathways <- read_rds(fs::path(dir_temp,
                                                   "cpd_mz_matching", 
...
# Calculate how many mz per cpd and how many cpd per mz
x1 <- x1 %>% 
  group_by(metabolite, mz, rt) %>% 
  mutate(n_cpds_per_mz = length(mz)) %>% 
  ungroup() %>% 
  group_by(cpd) %>% 
  mutate(n_mz_per_cpd = length(mz)) %>% 
  ungroup()

# Collapse across cpds
x2 <-  x1 %>% 
  group_by(metabolite, mz, rt) %>% 
...
solar_single_matches_w_mwas <- left_join(x2, 
                                         mwas_results %>% 
                                           mutate(mz = as.character(mz))) %>% 
...
write_rds(solar_single_matches_w_mwas, fs::path(dir_temp, 
