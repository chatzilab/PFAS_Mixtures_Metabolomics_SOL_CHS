# plot PIPs for figure 4
# have to run 2_6 prior to this script

# Subset SOLAR and CHS
sol <- sig_ecs_reduced %>% 
  select(met_name_tyr_pw:mass_diff, contains("sol")) %>% 
...
chs <- sig_ecs_reduced %>% 
  select(met_name_tyr_pw:mass_diff, contains("chs")) %>% 
...
(sol <- ggplot(sol_data, 
   ...
   facet_grid(tyr_subpath~cohort, scales = "free",
              space = "free") +
...
# Combine Plots -----------------------
(tyr_met_pips <- plot_grid(NULL, sol, 
                      NULL, chs, 
                      ncol = 1, align = "v", 
...
#                            "Figure S1 PFAS and tyr metabolite pips.jpg"),
#        width = 6.7, height = 9)
...
length(unique(chs_data$met_name_tyr_pw))
