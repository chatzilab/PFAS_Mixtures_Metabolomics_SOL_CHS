...
chs_mzkey_w_superpathways <- read_rds(here::here("Temporary results",
                                                   "cpd_mz_matching", 
...
# chs_single_matching_only <- one_cpd_mult_mz %>% 
#   group_by(mz) %>% 
...
chs_single_matches_w_mwas <- left_join(chs_single_matching_only, 
                                         mwas_results %>% 
                                           mutate(mz = as.character(mz))) %>% 
