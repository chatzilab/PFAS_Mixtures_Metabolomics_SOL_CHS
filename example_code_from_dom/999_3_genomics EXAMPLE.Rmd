...
## NAFLD Injury: Sex-Specific ALT

## General Liver Injury

Within each omics layer, we will use the general linear model to screen for omics features that are associated with disease outcomes of liver injury.

```{r}
# general liner model of dependent variable on independent var, adjusting for covariates
lm_y_x_cov <- function(dep_var, indep_var){

  # formula for univariate linear regression
  lin_reg_formula <- as.formula(paste0(dep_var, " ~ ", indep_var, " + ", paste0(collapse = " + ", names_covars)))
  
  multivar_reg <- df_selection_messy %>% lm(formula = lin_reg_formula) 
  multivar_reg_sum <- summary(multivar_reg)
  p_val_beta <- multivar_reg_sum$coefficients[2,"Pr(>|t|)"]
...
get_lmyxcov_byclass <- function(class_prefix, dep_var){
  purrr::map_df(
    .x = get_names_byclass(class_prefix), 
    .f = lm_y_x_cov, dep_var = dep_var) %>% 
    dplyr::mutate(
...
# get_lmyxcov_byclass("pro_", "rs1976403_C")

# get p-value for linear regression adjusting for covariates of combinations of all outcome variables in a class on all exposure variables in a second class
get_lmyxcov_by2class <- function(dep_class_prefix, indep_class_prefix){
  purrr::map_df(
    .x = get_names_byclass(dep_class_prefix), 
    .f = ~ purrr::map_df(.x = get_names_byclass(indep_class_prefix), .f = lm_y_x_cov, dep_var = .x)
    ) %>% 
    dplyr::mutate(
      # adjust p-value for multiple comparisons
      p_val_fdr = p.adjust(p_val_beta, method = "BH"),
...
# target_snp_pro <- get_lmyxcov_by2class(dep_class_prefix = "rs", indep_class_prefix = "pro_")
```


## Genetic SNPs

SNPs related to liver injury were identified from a literature search.  We will screen for omics features that are significantly related to any of these SNPs.

$$
Y_\text{Liver Injury SNPs} = \alpha + \beta_1X_\text{Omics Feature} 
     + \color{red}{\beta_iX_{i\text{ Child Sociodemographics}}}  
$$
...
```{r}
# regress all 122 SNPs on all 36 inflammatory proteins
# target_snp_pro <- get_lmyxcov_by2class(dep_class_prefix = "rs", indep_class_prefix = "pro_")
# save(target_snp_pro, file = paste0(dir_data_secure, "/target_snp_pro_", Sys.Date(), ".Rda"))
load(paste0(dir_data_secure, "/target_snp_pro_2020-12-18.Rda"))

# features with significant associations BEFORE adjusting for multiple comparisons
target_snp_pro_sig <- target_snp_pro %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_snp_pro_sig_fdr <- target_snp_pro %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_snp_pro %>% nrow()` **Total** pairwise comparisons of `r length(get_names_byclass("rs"))` SNPs on `r length(get_names_byclass("pro_"))` proteins.

`r target_snp_pro_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_snp_pro_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

#### Significant FDR Adjusted
```{r}
target_snp_pro_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
...
```{r}
# regress all 122 SNPs on all 177 serum metabolites
# target_snp_metser <- get_lmyxcov_by2class(dep_class_prefix = "rs", indep_class_prefix = "metser_")
# save(target_snp_metser, file = paste0(dir_data_secure, "/target_snp_metser_", Sys.Date(), ".Rda"))
load(paste0(dir_data_secure, "/target_snp_metser_2020-12-18.Rda"))

# features with significant associations BEFORE adjusting for multiple comparisons
target_snp_metser_sig <- target_snp_metser %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_snp_metser_sig_fdr <- target_snp_metser %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_snp_metser %>% nrow()` **Total** pairwise comparisons of `r length(get_names_byclass("rs"))` SNPs on `r length(get_names_byclass("metser_"))` serum metabolites

`r target_snp_metser_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_snp_metser_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

#### Significant FDR Adjusted
```{r}
target_snp_metser_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
```
...
```{r}
# regress all 122 SNPs on all 44 serum metabolites
# target_snp_meturi <- get_lmyxcov_by2class(dep_class_prefix = "rs", indep_class_prefix = "meturi_")
# save(target_snp_meturi, file = paste0(dir_data_secure, "/target_snp_meturi_", Sys.Date(), ".Rda"))
load(paste0(dir_data_secure, "/target_snp_meturi_2020-12-18.Rda"))

# features with significant associations BEFORE adjusting for multiple comparisons
target_snp_meturi_sig <- target_snp_meturi %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_snp_meturi_sig_fdr <- target_snp_meturi %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_snp_meturi %>% nrow()` **Total** pairwise comparisons of `r length(get_names_byclass("rs"))` SNPs on `r length(get_names_byclass("meturi_"))` urine metabolites.

`r target_snp_meturi_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_snp_meturi_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

#### Significant FDR Adjusted
```{r}
target_snp_meturi_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "100px")
```
...
Enzymes related to liver injury include ALT, AST, and GGT. We will screen for omics features that are significantly related to any of these enzymes.

### ALT

$$
Y_\text{ALT} = \alpha + \beta_1X_\text{Omics Feature} 
     + \color{red}{\beta_iX_{i\text{ Child Sociodemographics}}}  
$$
...
$$
Y_\text{AST} = \alpha + \beta_1X_\text{Omics Feature} 
     + \color{red}{\beta_iX_{i\text{ Child Sociodemographics}}}  
$$
...
$$
Y_\text{GGT} = \alpha + \beta_1X_\text{Omics Feature} 
     + \color{red}{\beta_iX_{i\text{ Child Sociodemographics}}}  
$$
...
target_alt_pro <- get_lmyxcov_byclass(dep_var = "alt_n", class_prefix = "pro_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_alt_pro_sig <- target_alt_pro %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_alt_pro_sig_fdr <- target_alt_pro %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_alt_pro %>% nrow()` **Total** pairwise comparisons of continuous ALT on `r length(get_names_byclass("pro_"))` proteins.

`r target_alt_pro_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_alt_pro_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_alt_pro_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
...
```{r}
# regress AST on all 36 inflammatory proteins
target_ast_pro <- get_lmyxcov_byclass(dep_var = "ast_n", class_prefix = "pro_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_ast_pro_sig <- target_ast_pro %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_ast_pro_sig_fdr <- target_ast_pro %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_ast_pro %>% nrow()` **Total** pairwise comparisons of continuous AST on `r length(get_names_byclass("pro_"))` proteins.

`r target_ast_pro_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_ast_pro_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_ast_pro_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "100px")
```
...
```{r}
# regress GGT on all 36 inflammatory proteins
target_ggt_pro <- get_lmyxcov_byclass(dep_var = "ggt_n", class_prefix = "pro_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_ggt_pro_sig <- target_ggt_pro %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_ggt_pro_sig_fdr <- target_ggt_pro %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_ggt_pro %>% nrow()` **Total** pairwise comparisons of continuous GGT on `r length(get_names_byclass("pro_"))` proteins.

`r target_ggt_pro_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_ggt_pro_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_ggt_pro_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
```
...
```{r}
# regress ALT on all 177 serum metabolites
target_alt_metser <- get_lmyxcov_byclass(dep_var = "alt_n", class_prefix = "metser_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_alt_metser_sig <- target_alt_metser %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_alt_metser_sig_fdr <- target_alt_metser %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_alt_metser %>% nrow()` **Total** pairwise comparisons of continuous ALT on `r length(get_names_byclass("metser_"))` serum metabolites

`r target_alt_metser_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_alt_metser_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_alt_metser_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "100px")
```
...
```{r}
# regress AST on all 177 serum metabolites
target_ast_metser <- get_lmyxcov_byclass(dep_var = "ast_n", class_prefix = "metser_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_ast_metser_sig <- target_ast_metser %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_ast_metser_sig_fdr <- target_ast_metser %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_ast_metser %>% nrow()` **Total** pairwise comparisons of continuous AST on `r length(get_names_byclass("metser_"))` serum metabolites

`r target_ast_metser_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_ast_metser_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_ast_metser_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
```
...
```{r}
# regress GGT on all 177 serum metabolites
target_ggt_metser <- get_lmyxcov_byclass(dep_var = "ggt_n", class_prefix = "metser_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_ggt_metser_sig <- target_ggt_metser %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_ggt_metser_sig_fdr <- target_ggt_metser %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_ggt_metser %>% nrow()` **Total** pairwise comparisons of continuous GGT on `r length(get_names_byclass("metser_"))` serum metabolites

`r target_ggt_metser_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_ggt_metser_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_ggt_metser_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
```
...
```{r}
# regress ALT on all 44 urine metabolites
target_alt_meturi <- get_lmyxcov_byclass(dep_var = "alt_n", class_prefix = "meturi_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_alt_meturi_sig <- target_alt_meturi %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_alt_meturi_sig_fdr <- target_alt_meturi %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_alt_meturi %>% nrow()` **Total** pairwise comparisons of continuous ALT on `r length(get_names_byclass("meturi_"))` urine metabolites.

`r target_alt_meturi_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_alt_meturi_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_alt_meturi_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "100px")
```
...
```{r}
# regress AST on all 44 urine metabolites
target_ast_meturi <- get_lmyxcov_byclass(dep_var = "ast_n", class_prefix = "meturi_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_ast_meturi_sig <- target_ast_meturi %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_ast_meturi_sig_fdr <- target_ast_meturi %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_ast_meturi %>% nrow()` **Total** pairwise comparisons of continuous AST on `r length(get_names_byclass("meturi_"))` urine metabolites.

`r target_ast_meturi_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_ast_meturi_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_ast_meturi_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "100px")
```
...
```{r}
# regress GGT on all 44 urine metabolites
target_ggt_meturi <- get_lmyxcov_byclass(dep_var = "ggt_n", class_prefix = "meturi_")

# features with significant associations BEFORE adjusting for multiple comparisons
target_ggt_meturi_sig <- target_ggt_meturi %>% filter(p_val_beta < alpha_pval_beta)

# features with significant associations AFTER adjusting for multiple comparisons
target_ggt_meturi_sig_fdr <- target_ggt_meturi %>% filter(p_val_fdr < alpha_pval_beta)
```

`r target_ggt_meturi %>% nrow()` **Total** pairwise comparisons of continuous AST on `r length(get_names_byclass("meturi_"))` urine metabolites.

`r target_ggt_meturi_sig %>% nrow()` **Significant Unadjusted** pairwise comparisons.

`r target_ggt_meturi_sig_fdr %>% nrow()` **Significant FDR Adjusted** pairwise comparisons.

##### Significant FDR Adjusted
```{r}
target_ggt_meturi_sig_fdr %>% 
  kable() %>% 
  kable_paper() %>%
  scroll_box(width = "100%", height = "100px")
```
...
```{r}
# dataframe of all SNPs that were significantly associated with omics features after adjusting for FDR
target_snp_sig_fdr <- bind_rows(
  target_snp_pro_sig_fdr,
  target_snp_metser_sig_fdr,
  target_snp_meturi_sig_fdr
) %>% 
  ...
  dplyr::mutate(
    # indicator for which targeted feature
    i_target = 1,
...
# dataframe of all SNPs that were significantly associated with omics features after adjusting for FDR
target_livinj_sig_fdr <- bind_rows(
  target_alt_pro_sig_fdr,
  target_ast_pro_sig_fdr,
  target_ggt_pro_sig_fdr,
  target_alt_metser_sig_fdr,
  ...
  dplyr::mutate(
    ...
    dep_var = case_when(dep_var == "alt_n" ~ 1, dep_var == "ast_n" ~ 2, dep_var == "ggt_n" ~ 3) %>% factor(1:3, c("ALT", "AST", "GGT")),

  )
```

```{r}
# combined dataset of all targeted features identified by significant FDR-adjusted linear regression with SNP, ALT, AST, or GGT
target_sig_fdr <- bind_rows(
  target_snp_sig_fdr,
  target_livinj_sig_fdr
) %>% 
  # create indicator factor for dependent variable identified by 
  dplyr::mutate(
    i_target = ifelse(i_target == 2, no = i_target, yes = case_when(
      dep_var == "ALT" ~ 2,
      dep_var == "AST" ~ 3,
      dep_var == "GGT" ~ 4
    )),
    i_target = factor(i_target, 1:4, c("SNP", "ALT", "AST", "GGT"))
  )
```
...
target_snp_sig_fdr_heatmap_messy <- target_snp_sig_fdr %>%
...
target_snp_sig_fdr_heatmap <- target_snp_sig_fdr_heatmap_messy %>% 
  select(-omics_group, -indep_var) %>% # eliminate meta-info
  as.matrix()

# heatmap only accepts rownames, not column of rownames
rownames(target_snp_sig_fdr_heatmap) <- target_snp_sig_fdr_heatmap_messy$indep_var
...
logpval_hotter_palette <- colorRampPalette(c("white", "orange", "red")) (n = 1000)

gplots::heatmap.2(
  target_snp_sig_fdr_heatmap, col = logpval_hotter_palette, key = TRUE, trace = "none", scale = "none",
  # add spectrum indicating which omics class to which each feature belongs
  RowSideColors = colors_fct_omics_dark[as.character(target_snp_sig_fdr_heatmap_messy$omics_group)],
...
```{r}
# all SNPs
target_snp_sig_fdr %>% manhattan_facet(xaxis_txt_size = 2)
```
...
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs1535_G") %>% manhattan_facet()
```
...
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs174601_T") %>% manhattan_facet()
```

##### rs10883437_A
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs10883437_A") %>% manhattan_facet()
```

##### rs10883451_C
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs10883451_C") %>% manhattan_facet()
```

##### rs11591741_C
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs11591741_C") %>% manhattan_facet()
```

##### rs11597086_C
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs11597086_C") %>% manhattan_facet()
```

##### rs11597390_A
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs11597390_A") %>% manhattan_facet()
```

##### rs1175543_G
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs1175543_G") %>% manhattan_facet()
```

##### rs12784396_T
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs12784396_T") %>% manhattan_facet()
```

##### rs13412852_T
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs13412852_T") %>% manhattan_facet()
```

##### rs17668255_T
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs17668255_T") %>% manhattan_facet()
```

##### rs17668357_C
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs17668357_C") %>% manhattan_facet()
```

##### rs17729876_A
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs17729876_A") %>% manhattan_facet()
```

##### rs579459_C
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs579459_C") %>% manhattan_facet()
```

##### rs657152_A
```{r}
# single SNP
target_snp_sig_fdr %>% filter(dep_var == "rs657152_A") %>% manhattan_facet()
```
...
```{r}
# prepare data for heatmap
target_livinj_sig_fdr_heatmap_messy <- target_livinj_sig_fdr %>%
  # vars useful for heatmap
  dplyr::select(dep_var, indep_var, omics_group, p_val_fdr) %>%
  # transform p-values to -log(p-value, FDR-adjusted)
  mutate(p_val_fdr = -log(p_val_fdr)) %>% 
...
##### All Liver Injury Enzymes
```{r}
target_livinj_sig_fdr %>% manhattan_facet(xaxis_txt_size = 2)
```

##### ALT
```{r}
target_livinj_sig_fdr %>% filter(dep_var == "ALT") %>% manhattan_facet()
```

##### AST
```{r}
target_livinj_sig_fdr %>% filter(dep_var == "AST") %>% manhattan_facet()
```

##### GGT
```{r}
target_livinj_sig_fdr %>% filter(dep_var == "GGT") %>% manhattan_facet()
```
...
get_target_features <- function(i_omics, df = target_sig_fdr){
  tar_feat_vec <- df %>% 
...
pcor_tarfeat_allfeat <- function(i_omics){
  
  # get list of target features
  tar_feat <- get_target_features(i_omics)
...
load(paste0(dir_data_secure, "/partial_corr_target_features_2020-12-18.Rda"))
```



# Circos Globe Plots{.tabset}
...
p_circos_omics <- function(omics_layer){
  ...
  names_tar_feat <- clean_punct(clean_feat_aa_ae(clean_feat_labs(unique(pcor_tar_feat[[paste0("pcor_tar_", i_params$prefix)]]$tar_feat))))
  
  # single dataframe for circlize plot
  df_messy <- pcor_tar_feat[[paste0("pcor_tar_", i_params$prefix)]] %>% 
    ...
    bind_rows(data.frame(
      ...
      class = metser_class[names_tar_feat]
      ))
