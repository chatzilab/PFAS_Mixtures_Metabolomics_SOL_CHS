...
C18_MWAS_reg = function(exposure, met_name){
  ...
  solar_exposure_outcome$tanner + 
  solar_exposure_outcome$ses + 
  solar_exposure_outcome$wave'
  formula = paste0('solar_c18$',
                   '`',met_name,'`',
                   '~',
...
input_data = cbind.data.frame(rep(exposures, 
                                  each = ncol(solar_c18)-1 ), 
                              rep(colnames(solar_c18)[2:ncol(solar_c18)], 
                                  length(exposures)))
...
input_data = cbind.data.frame(rep(exposures,
                                  each = ncol(solar_hilic)-1), 
                              rep(colnames(solar_hilic)[2:ncol(solar_hilic)], 
                                  length(exposures))  )
names(input_data) = c('exposure','met_name')
