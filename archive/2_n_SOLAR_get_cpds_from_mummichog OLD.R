...
SOLAR_c18_set <- read_rds(here::here("Temporary results",
                                     folders[i], "SOLAR",
                                     "c18", 
                                     paste0(folders[i],
                                            "_c18_mumichog_results.rds")))
# Get MZ and retention time key
mzkey_c18 <- SOLAR_c18_set$mz2cpd_dict
...
output <- as.data.frame(SOLAR_c18_set$mummi.resmat) %>%
  rownames_to_column(., var = "name") %>%
  janitor::clean_names()

# Get name of paths
paths <- tibble(name = SOLAR_c18_set$path.nms,
                path.hits = SOLAR_c18_set$path.hits)

# Merge name of pathways and p values from output
output1 <- inner_join(paths, output) 

# Filter only pathways with >1 match
output1 <- output1 %>%  
...
solar_mzkey_c18_w_superpathways <- inner_join(mzkey_c18, 
...
x1_c18 <- left_join(c18_mwas_min_p_vals %>% 
                    ...
                    solar_mzkey_c18_w_superpathways) %>% 
  filter(!is.na(cpd))
...
write_csv(c18_single_matching_only, 
          here::here("Temporary results", 
                     "SOLAR_C18_mz_cpd_superpathway_key.csv"))
...
x1_hilic <- left_join(hilic_mwas_min_p_vals %>% 
                        mutate(mz = as.character(mz)), 
                      solar_mzkey_hilic_w_superpathways) %>% 
  filter(!is.na(cpd))
...
write_csv(hilic_single_matching_only, 
          here::here("Temporary results", 
                     "SOLAR_hilic_mz_cpd_superpathway_key.csv"))
