...
# read hand curated name conversions from metaboanalyst
# Read in metaboanalyst key 
name_conversions_metaboanalyst <- readxl::read_xlsx(file_location, 
                                                    sheet = "metaboanalyst", 
                                                    na = "NA") %>% 
  clean_names() %>% 
...
# Read in metanet key
name_conversions_metanet <- readxl::read_xlsx(file_location, 
                                              sheet = "MetaNetX") %>% 
  clean_names() %>% 
  rename(chem_id = number_query)

# Read in mbrole name conversions
name_conversions_mbrole <- readxl::read_xlsx(file_location, 
                                             sheet = "mbrole") %>% 
  clean_names()  %>% 
...
# Modify mbrole 
mbrole_w <- name_conversions_mbrole %>% 
...
name_cpd_key <- full_join(name_conversions_metaboanalyst, 
                                 mbrole_w, by = "chem_id", 
                                 suffix = c("_metab", "_mbrole")
                                ) %>%
  full_join(name_conversions_metanet,  by = "chem_id", suffix = c("", "_metanet"))

# Merge matgching id databases 
name_cpd_key2 <- name_cpd_key %>% 
 ...
 select(chem_id, match, name_2, hmdb, everything()) 


table(name_cpd_key2$hmdb_metab == name_cpd_key2$hmdb_mbrole)
